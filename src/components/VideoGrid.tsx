      ...
      >
        {videos.map((vid) => {
          ...
          const dateStr = vid.createdAt 
            ? new Date(vid.createdAt).toLocaleDateString('bn-BD', { day: 'numeric', month: 'short' })
            : "আজ";
          ...
          return (
            <div
              ...
              className="bg-[#1a1a1a] rounded-lg overflow-hidden group border border-[#222222] cursor-pointer hover:border-[#f5c518] active:border-[#f5c518] transition-all duration-200 flex flex-col justify-between"
            >
              ...
              <div className="relative aspect-[16/9] w-full bg-[#0f0f0f] overflow-hidden">
                ...
                {onToggleBookmark && (
                  <button
                    onClick={(e) => onToggleBookmark(vid.id, e)}
                    className="absolute top-1.5 right-1.5 z-20 p-1 rounded-full bg-black/60 hover:bg-black/90 text-[#aaaaaa] hover:text-[#f5c518] border border-[#222222] transition-transform active:scale-95"
                ...
                {/* Continue Watching Linear Progress Bar on Thumbnail bottom */}
                {progress > 0 && (
              ...
              <div className="p-2.5 flex flex-col justify-between flex-grow text-left">
                {/* Title (2 lines max as requested) */}
                <h3 className="text-white text-xs font-bold font-sans tracking-tight line-clamp-2 leading-snug group-hover:text-[#f5c518] transition-colors h-[2.5rem]">
                  {vid.title}
                </h3>
                
                {/* Footer views and date info */}
